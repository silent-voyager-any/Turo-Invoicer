...
No `cookies`, `webRequest`, `tabs`, `scripting`, `offscreen`, `declarativeNetRequest`, `unlimitedStorage`, or all-sites permission is declared. `activeTab` is used only after the user opens the popup on the visible E-ZPass Transactions tab; `alarms` removes sent evidence after its retention deadline. Static scripts and matching-tab access use the two portal host permissions. Version 0.5.12 binds each screenshot request to a short-lived in-memory token and the exact active transaction tab. Normal E-ZPass syncing creates a temporary inactive tab on the already-allowlisted portal, validates every destination URL, and closes it in `finally`; evidence uses and restores the visible tab. PNG blobs stay in IndexedDB; only hashes and reduced metadata enter extension storage. A partial run stores only normalized verified rows and separately retains the previous complete normalized snapshot; it never stores raw portal responses or account data.
