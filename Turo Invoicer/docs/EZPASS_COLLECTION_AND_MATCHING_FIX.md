...
Version 0.4.4 collects Turo first, derives the required Transaction Date range from completed trips, commits that range through the signed-in portal's masked inputs, and walks every result page. Collection is complete only after the portal exposes a disabled Next control (or explicitly reports an empty range). A collector revision handshake prevents a stale injected content script from being mistaken for the current build.
