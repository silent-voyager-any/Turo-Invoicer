...
The filtered collector accepts a stably rendered, nonempty result with no pager as one page. E-ZPass also removes its pager when **View 100** makes every result fit on one page; the collector must re-read the live table after that change. If the page-size transition is uncertain, or a visible pager is missing Previous or Next, collection fails safely instead of saving a first-page-only total. Retry sync after the portal settles; the previous synchronized snapshot remains intact for pagination-proof failures.
...
Version 0.5.12 no longer types dates or selects per-query form controls. It opens one temporary inactive E-ZPass tab, waits directly for the labelled Tag/Plate control and its hydrated list, then loads an exact direct-query URL for each trip. The inventory path does not require the date, Type, or Search controls. If an older extension reports that the Tag/Plate filter was not found, reload the unpacked extension and both portal tabs. Drawer, control, replacement, listbox, missing-identifier, redirect, mismatched-result, and stalled-page failures are reported separately; partial matches and dropped leading zeros remain invalid.

The direct-query collector requires stable matching rows or a sustained empty result. A stalled URL is reloaded once; if it still fails, the affected trip shows `search_incomplete`. Other verified trip results remain visible, but the incomplete trip cannot enter evidence or a batch. Redirects, unexpected parameters, ambiguous inventory matches, and out-of-range or wrong-identifier rows fail closed and preserve the prior verified state.
