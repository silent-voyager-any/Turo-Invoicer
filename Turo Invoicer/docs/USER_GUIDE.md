...
The dashboard builds one card per discovered Turo vehicle. The vehicle name and current registration plate are shown prominently; the numeric Turo internal ID appears only as a diagnostic and is not an E-ZPass tag. Choose **Refresh E-ZPass list** to load the exact tags and plates offered by the signed-in account without changing synchronized trips or tolls. Link a verified plate directly, or choose an account identifier and vehicle. Add inclusive effective dates when an identifier changes vehicles; leave a boundary empty for an open-ended range.

Turo is not assumed to provide E-ZPass tag numbers. Prefer the verified account list. Manual entry remains available for recovery and historical identifiers, but it must use the complete value from your own E-ZPass records. Leading zeros remain significant, while letter case, spaces, punctuation, and an explicit `NY:`-style plate prefix do not affect comparison. Partial and fuzzy matches are never used.
...
| Confirm vehicle | Time matches one trip, but the captured identifier is not mapped | Open the inline Needs Review editor, verify the vehicle, tag/plate type, identifier and effective dates, then save. Cached matches and Batch recalculate immediately. |
