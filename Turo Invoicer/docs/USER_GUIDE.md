...
Do not leave duplicate matching history/transactions tabs open. Version 0.5.4 searches E-ZPass separately for every eligible trip and confirmed active tag/plate, binds each calendar to its Start or End Date field, and restores the filters that were present before sync. If the verified session-expiry warning appears during a user-started sync, the extension chooses **continue working**; it does not keep sessions alive otherwise, solve challenges, or sign in for you.
