...
Do not leave duplicate matching history/transactions tabs open. Version 0.5.7 searches E-ZPass separately for every eligible trip and confirmed active tag/plate, types the trip dates into the masked `MM/DD/YY` fields, and restores the filters that were present before sync. If E-ZPass no longer offers a configured tag or plate, update or remove that assignment on Vehicles and sync again; other identifier searches can continue, but affected trips remain blocked. It does not use the calendar widget. If the verified session-expiry warning appears during a user-started sync, the extension chooses **continue working**; it does not keep sessions alive otherwise, solve challenges, or sign in for you.
