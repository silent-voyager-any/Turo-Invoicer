...
Version 0.4.6 collection runs may also contain `range`, `requestedRange`, `observedRange`, `completeForRange`, `ordering`, and `terminalReason`. E-ZPass completeness requires disabled Next or a fully older page under explicit descending-sort proof and monotonic observed boundaries. A record count alone never establishes completeness. Turo's derived range uses verified uncharged trips when available and otherwise falls back to completed trips with unknown invoice status.
