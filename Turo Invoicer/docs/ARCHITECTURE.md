...
Version 0.5.6 builds one E-ZPass search per eligible trip and active confirmed identifier. The content script locates the transaction filter through accessible labels, types six digits into each masked Start or End Date field, and verifies the exact normalized date without opening the calendar widget. A stable nonempty response with no pagination navigation is terminal proof for a single-page query; if a pager is visible, its required controls and page advancement remain mandatory. During an explicit collection it may activate the unique **continue working** action in the verified session-expiry dialog; no keep-alive runs outside collection. It restores the original filters in `finally`. `Lane Txn ID` deduplicates tolls; any failed query, restoration, repeated page, route change, or timeout preserves the prior complete snapshot.
