...
Version 0.4.3 gives explicit E-ZPass syncs a stricter path. After Turo supplies completed-trip coverage, the E-ZPass content script divides that inclusive range into 14-day chunks, drives the visible Transaction Date filter, and reads sequential pages. `Lane Txn ID` deduplicates normalized tolls. A collector revision handshake rejects stale injected scripts. The run is complete only when every chunk reaches the verified terminal condition; any stalled/repeated page, missing pager, route change, timeout, or cap preserves the prior snapshot.
