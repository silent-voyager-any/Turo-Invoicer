...
Version 0.4.4 collects Turo first, derives the required E-ZPass Transaction Date range from completed trips, hydrates the portal's masked date inputs through typing-compatible events, and walks results until the disabled Next control proves completion. It also detects an E-ZPass tab that still has an older injected collector. Turo pagination and invoice status remain fail-closed; evidence and submission are not yet enabled. Reload the extension and both portal tabs after updating.
