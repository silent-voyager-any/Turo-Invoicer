...
### Waiting for Turo's SPA

`COLLECT_NOW` for Turo now holds the Chrome message channel open for up to
20 seconds. The shared MutationObserver watches inserted nodes, text changes,
and hydration attributes such as `datetime`, `href`, and `data-vehicle-id`.
It re-runs the Turo selectors until complete records appear; network responses
can satisfy the same wait. A 300 ms unchanged-record window lets small render
batches settle without being starved by unrelated DOM activity. The worker
allows 25 seconds for this response and does not reconcile until it arrives.

This fixes the earlier immediate-empty reply: an observer already existed, but
the sync request did not wait for its later captures. Timeout/navigation returns
an actionable error and preserves the last complete saved snapshot. All pending
wait timers are cleared on completion, clear, or navigation. E-ZPass's existing
collection behavior and the pure reconciler are unchanged.

After updating, reload the extension on `chrome://extensions`, then reload the
Turo tab and visit `/us/en/host/trips`. No manifest or permission changes are
needed. If trip cards are visible but the new wait still times out, inspect a
redacted card fixture: the fallback selectors cannot reconstruct missing vehicle
IDs or times, and are not a guarantee of the current authenticated page's schema.
